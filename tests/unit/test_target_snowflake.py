...
class TestTargetSnowflake(unittest.TestCase):
    ...
    def test_persist_lines_with_40_records_and_batch_size_of_20_batch(self, dbSync_mock,
                                                                                     flush_streams_mock):
        ...
        batch_message = {
            ...
            "manifest": [
                "file://tests/unit/resources/batch.1.jsonl.gz",
                "file://tests/unit/resources/batch.2.jsonl.gz",
            ],
        }

        tap_output = [json.dumps(schema_message), json.dumps(batch_message)]
        ...
        target_snowflake.persist_lines(self.config, tap_output)

        self.assertEqual(2, flush_streams_mock.call_count)
